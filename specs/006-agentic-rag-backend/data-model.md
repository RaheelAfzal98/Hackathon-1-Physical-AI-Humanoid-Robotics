...
- `thread_id`: String - Associated OpenAI Assistant thread ID
...
- `retrieval_top_k`: Integer - Number of results to retrieve from Qdrant (default: 5)
...
**Fields**:
- `query_text`: String - The actual question or query text
- `session_id`: String? - Optional session ID for stateful interactions
- `user_context`: Dict? - Additional context about the user or situation
- `response_options`: Dict? - Specific options for this query (overrides session config)
...
- `content`: String - The main response text from the agent
...
- `query_time_ms`: Integer - Time taken to generate the response
...
- `content_preview`: String - Short preview of the referenced content
...
### RetrievedContent
**Description**: Content chunks retrieved from Qdrant with their metadata that ground agent responses

**Fields**:
- `content_id`: String - Identifier for the content chunk
...
- `page_title`: String - Title of the document
...
- Grounding strictness must be between 0.0 and 1.0
...
### Primary Indexes
- AgentSession.session_id (unique)
- AgentResponse.response_id (unique)
- AgentResponse.session_id (for session-based queries)

### Secondary Indexes
- AgentSession.last_interaction (for session cleanup)
- AgentSession.created_at (for retention policies)

### Performance Optimizations
- Cache frequently retrieved content chunks
- Implement efficient similarity thresholds
- Use vector database for fast retrieval
- Implement proper pagination for long conversation histories